...
class War(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        # Боевой цикл запустится в cog_load
    ...
    async def battle_loop(self):
        ...
        for war in active_wars:
            ...
            if attacker['army_count'] <= 0 or defender['army_count'] <= 0:
                ...
                # Отправить новость о победе (можно добавить, но пока опустим)
    ...
    async def _execute_war_declaration(self, interaction, attacker, defender, is_bot=False, target_user=None):
        ...
        if not is_bot and defender['owner_id']:
            ...
            if target_user:
                try:
                    await target_user.send(f"{interaction.user.mention} объявил вам войну от страны **{attacker_name}**!")
                except discord.Forbidden:
                    pass
